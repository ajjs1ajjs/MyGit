      ...
      <div class="card">
        <div class="card-body !p-6">
          <form @submit.prevent="go" class="space-y-4">
            <div><label for="login-username" class="text-xs font-medium text-[#171717] dark:text-[#fafafa] block mb-1.5">Username</label><input id="login-username" name="username" v-model="login" type="text" required autocomplete="username" placeholder="admin" /></div>
            <div><label for="current-password" class="text-xs font-medium text-[#171717] dark:text-[#fafafa] block mb-1.5">Password</label><input id="current-password" name="password" v-model="pass" type="password" required autocomplete="current-password" /></div>
...
const r = useRouter(); const a = useAuthStore(); const login=ref(""); const pass=ref(""); const err=ref("");
async function go(){ try{ const u = await a.login(login.value,pass.value); if(u.must_change_password) r.push("/auth/change-password"); else r.push("/") } catch(e:any){ err.value=e.message } }
</script>
