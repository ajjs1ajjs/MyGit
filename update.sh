...
cd frontend && npm install --no-audit --no-fund 2>/dev/null || npm install
npm run build && sudo rm -rf /opt/mygit/static/frontend && sudo mkdir -p /opt/mygit/static/frontend && sudo cp -r dist/* /opt/mygit/static/frontend/
